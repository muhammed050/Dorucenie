...
function backoff(attempt: number) { return Math.min(60 * 60 * 1000, 60_000 * 5 ** Math.max(0, attempt - 1)); }
...
export async function POST(request: Request) {
  ...
  for (const job of (jobs ?? [])) {
    ...
    const heartbeat = setInterval(async () => {
      if (!stopped) await db.rpc("heartbeat_job", { p_job_id: job.id, p_worker_id: workerId, p_lease_minutes: LEASE_MINUTES });
    }, 60_000);
    try {
      if (job.type === "TRACK_SHIPMENT") {
        ...
        await db.from("shipments").update({ status: tracking.status, estimated_delivery_at: tracking.estimatedDeliveryAt ?? null }).eq("id", payload.shipmentId).eq("organization_id", job.organization_id);
        for (const event of tracking.events) await db.from("shipment_events").upsert({ organization_id: job.organization_id, shipment_id: payload.shipmentId, provider_event_id: event.id ?? `${event.occurredAt}:${event.status}`, status: event.status, occurred_at: event.occurredAt, location: event.location ?? null, description: event.description ?? null, raw_event: event.raw }, { onConflict: "shipment_id,provider_event_id" });
